...
Mamba sweeps, and a gate-only fusion module with auxiliary
...
attains Precision@1 of $85.5\%$ Gaussian, $87.2\%$ power-line, and
$85.1\%$ EMG with Stage-I SI-SNR $12.15$/$32.41$/$13.83$\,dB. Against
same-protocol MindID/BrainNet baselines, V4 leads on Gaussian and EMG,
while MindID is slightly higher on power-line ($88.1\%$).
Held-out-subject AUROC $\approx 0.50$ motivates calibration as future work.
\end{abstract}

\begin{IEEEkeywords}
EEG biometrics, denoising autoencoder, state-space models, Mamba,
multimodal fusion, gate-only fusion, metric learning.
\end{IEEEkeywords}
...
we fuse the two embeddings with a \emph{gate-only fusion}
...
Precision@1 among V1--V4 on every noise family and reaches up to
$32.4$\,dB SI-SNR on power-line noise. We additionally re-implement
MindID and BrainNet as same-protocol baselines rather than comparing
only against source-paper numbers.
...
denoiser with a Mamba spectrogram branch and gate-only fusion for
...
and use a learned gate to combine the two views, instead
...
augments the second stage with a spectrogram branch and gate-only
fusion.
...
\textbf{Gate-only fusion.} The two unit-norm vectors are mixed by a
learned gate
$g=\sigma(W_g[e_{\mathrm{eeg}};e_{\mathrm{spec}}])\in(0,1)^{128}$,
where $\sigma$ is the elementwise sigmoid and
$W_g\in\mathbb{R}^{128\times 256}$ is a learned linear projection
acting as a soft per-coordinate selector between the two views,
\begin{equation}
e_{\mathrm{mix}}=g\odot e_{\mathrm{eeg}}
+(1-g)\odot e_{\mathrm{spec}},
\end{equation}
...
gate-only fusion module fuses the two L2-normalized
...
gate-only fusion with auxiliary branch losses.
\end{itemize}
In addition, \textbf{V5} re-implements two cited prior-work encoders
under the same AEP-Hybrid protocol: MindID with an ArcFace head and
BrainNet with a FaceNet-style triplet head. Both V5 baselines share the
WaveNet denoiser without Mamba, so the comparison isolates the
encoder/head under the same split, noise families, and three-seed
evaluation.

\textbf{Implementation.} All three-seed V1, V2, V4, and V5 experiments run
...
for V4. The V4 model totals $22.6$\,M parameters with end-to-end
inference latency of $187$--$199$\,ms per $4$\,s epoch on an RTX~5090
...
spectrogram branch, and gate-only fusion module account for the latency
...
clearest margins on Gaussian and EMG. V3 reaches a competitive
...
& \textbf{V4} \\
\midrule
Gaussian   & 0.822 & 0.798 & 0.749 & \textbf{0.855} \\
Power-line & 0.860 & 0.858 & 0.869 & \textbf{0.872} \\
EMG        & 0.824 & 0.811 & 0.758 & \textbf{0.851} \\
\bottomrule
\end{tabular}
...
\input{tables/priorwork}

\subsection{Same-Protocol Prior-Work Baselines}
Table~\ref{tab:v5} reports the V5 MindID and BrainNet baselines trained
under the same AEP-Hybrid protocol as the proposed model. MindID is a
strong same-protocol baseline: V4 remains higher on Gaussian and EMG
P@1, while MindID is slightly higher on power-line. BrainNet obtains
higher AUROC in this implementation but substantially lower retrieval
P@1, indicating that its score distribution is less aligned with nearest
neighbor identification under the current split.

\begin{table}[t]
\centering
\caption{V5 same-protocol MindID/BrainNet baselines, mean$\pm$std over
three seeds. V4 gate-only R34+Arc P@1 is shown for context.}
\label{tab:v5}
\begin{threeparttable}
\scriptsize
...
\toprule
\textbf{Noise} & \textbf{Model} & \textbf{P@1} & \textbf{AUROC} & \textbf{EER} \\
\midrule
\multirow{3}{*}{Gauss.}
 & V4 R34+Arc & \textbf{.855$\pm$.021} & .541$\pm$.102 & \textbf{.359$\pm$.010} \\
 & MindID+Arc & .814$\pm$.049 & .515$\pm$.016 & .367$\pm$.026 \\
 & BrainNet+Tri & .607$\pm$.049 & \textbf{.598$\pm$.107} & .363$\pm$.069 \\
\midrule
\multirow{3}{*}{P-line}
 & V4 R34+Arc & .872$\pm$.052 & .515$\pm$.079 & .340$\pm$.038 \\
 & MindID+Arc & \textbf{.881$\pm$.033} & .479$\pm$.065 & \textbf{.315$\pm$.017} \\
 & BrainNet+Tri & .650$\pm$.069 & \textbf{.603$\pm$.109} & .384$\pm$.047 \\
\midrule
\multirow{3}{*}{EMG}
 & V4 R34+Arc & \textbf{.851$\pm$.031} & .535$\pm$.075 & \textbf{.328$\pm$.056} \\
 & MindID+Arc & .840$\pm$.044 & .490$\pm$.016 & .344$\pm$.039 \\
 & BrainNet+Tri & .624$\pm$.029 & \textbf{.601$\pm$.123} & .361$\pm$.073 \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
\item Arc: ArcFace; Tri: triplet loss.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\midrule
\multirow{3}{*}{Gauss.}
 & R34+MS  & .799$\pm$.051 & .768$\pm$.058 & 12.15$\pm$.25 & .405$\pm$.041 & .402$\pm$.045 \\
 & R18+MS  & .790$\pm$.028 & .759$\pm$.038 & 12.15$\pm$.25 & .471$\pm$.026 & .381$\pm$.071 \\
 & R34+Arc & \textbf{.855$\pm$.021} & \textbf{.834$\pm$.016} & 12.15$\pm$.25 & \textbf{.541$\pm$.102} & \textbf{.359$\pm$.010} \\
\midrule
\multirow{3}{*}{P-line}
 & R34+MS  & .855$\pm$.031 & .825$\pm$.043 & 32.41$\pm$1.16 & .488$\pm$.044 & .375$\pm$.009 \\
 & R18+MS  & .840$\pm$.045 & .811$\pm$.052 & 32.41$\pm$1.16 & .482$\pm$.010 & .346$\pm$.024 \\
 & R34+Arc & \textbf{.872$\pm$.052} & \textbf{.844$\pm$.060} & 32.41$\pm$1.16 & \textbf{.515$\pm$.079} & \textbf{.340$\pm$.038} \\
\midrule
\multirow{3}{*}{EMG}
 & R34+MS  & .810$\pm$.054 & .788$\pm$.056 & 13.83$\pm$.34 & .483$\pm$.031 & .372$\pm$.038 \\
 & R18+MS  & .777$\pm$.047 & .741$\pm$.052 & 13.83$\pm$.34 & .477$\pm$.048 & .384$\pm$.063 \\
 & R34+Arc & \textbf{.851$\pm$.031} & \textbf{.828$\pm$.036} & 13.83$\pm$.34 & \textbf{.535$\pm$.075} & \textbf{.328$\pm$.056} \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
gate-only fusion (V2$\to$V4) is what unlocks retrieval gains of
$+5.7$, $+1.4$, and $+4.0$ percentage points respectively over V2, and
$+3.3$, $+1.2$, and $+2.7$ over V1. We do not attempt to attribute
...
$10.65/19.61/11.67$\,dB; V4: $12.15/32.41/13.83$\,dB on Gaussian /
...
\multirow{4}{*}{Gaussian}
 & V1: WaveNet  & 0.822 \\
 & V2: + Mamba  & 0.798 \\
 & V4: + Spec + Fusion & \textbf{0.855} \\
 & $\Delta$ V4$-$V1 & $+0.033$ \\
\midrule
\multirow{4}{*}{Power-line}
 & V1: WaveNet  & 0.860 \\
 & V2: + Mamba  & 0.858 \\
 & V4: + Spec + Fusion & \textbf{0.872} \\
 & $\Delta$ V4$-$V1 & $+0.012$ \\
\midrule
\multirow{4}{*}{EMG}
 & V1: WaveNet  & 0.824 \\
 & V2: + Mamba  & 0.811 \\
 & V4: + Spec + Fusion & \textbf{0.851} \\
 & $\Delta$ V4$-$V1 & $+0.027$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Where multimodal helps.} In the selected gate-only V4 run, the
largest mean-P@1 jumps occur on Gaussian and EMG noise, while power-line
still improves over V1/V2 but by a smaller margin. This suggests that
the spectrogram branch and gate help most when residual artifacts are
not captured by a single narrowband template; the $50$\,Hz power-line
case is already comparatively easy for the denoiser.
...
spectrogram branch, and gate-only fusion together — rather than an
...
unseen identities remains the open problem. The AUROC ($0.41$--$0.54$)
and EER ($0.33$--$0.40$) columns in Table~\ref{tab:v4} are computed on
...
study. (iv) We re-implement MindID and BrainNet under the same protocol,
but additional EEG-biometric baselines (EEGNet, ChronoNet, ICA+SVM) and
a cross-dataset transfer evaluation are out of scope here and are
scheduled for the journal version of this work; Table~\ref{tab:priorwork}
collects cited numbers under their own protocols as informational, not
...
a WaveNet+Mamba denoiser, a spectrogram Mamba branch, and a gate-only
fusion module trained with auxiliary branch losses.
Under the AEP-Hybrid protocol (Sec.~\ref{sec:setup}) on the PhysioNet
AEP dataset, V4 attains the highest mean Precision@1 among V1--V4
($85.5\%$/$87.2\%$/$85.1\%$). Against same-protocol V5 baselines, V4
leads on Gaussian and EMG, while MindID is slightly higher on
power-line; held-out-subject AUROC $\approx 0.50$ motivates calibration
as future work.
